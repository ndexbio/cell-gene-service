...
curl -H "Content-Type: application/json" --data @input/many_non_ex_cell.json http://localhost:8080/context/expression/cell_line > output/many_non_ex_cell_out.json


# Test case 6: singule cell single gene input
curl -H "Content-Type: application/json" --data @input/single_cell.json http://localhost:8080/context/expression/cell_line > output/single_cell_out.json
